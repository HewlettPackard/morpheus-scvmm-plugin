...
class ScvmmProvisionProvider extends AbstractProvisionProvider implements WorkloadProvisionProvider, HostProvisionProvider, ResourceProvisionProvider, ProvisionProvider.HypervisorProvisionFacet, HostProvisionProvider.ResizeFacet, WorkloadProvisionProvider.ResizeFacet, ProvisionProvider.BlockDeviceNameFacet {
	...
	/**
	 * Deploying to the cloud/cluster and letting SCVMM pick the volume is supported, so the datastore stays optional
	 * during provisioning (an "Auto" option is offered alongside the datastores).
	 */
	@Override
	...
	/**
	 * The root volume datastore is selectable for SCVMM; the list is scoped to the selected resource pool by
	 * {@link ScvmmCloudProvider#filterDatastores}.
	 */
	@Override
    ...
    def getHostAndDatastore(Cloud cloud, account, clusterId, hostId, Datastore datastore, datastoreOption, size, siteId = null, maxMemory, isClone=false) {
        ...
        if (!datastore) {
            ...
            // Restrict to storage the selected cluster can actually place on. zonePool is only read for datastores
            // synced before scoping moved to assignedZonePools.
            if (cluster) {
    ...
    /**
     * Datastore scoping moved to assignedZonePools, so the Cluster Shared Volume marker recorded during sync is what
     * determines HA placement. Datastores not yet re-synced fall back to the legacy "has a zonePool" behaviour.
     */
    ...
    ServiceResponse<ProvisionResponse> runHost(ComputeServer server, HostRequest hostRequest, Map opts) {
        ...
        try {
            ...
            if (layout && typeSet) {
                ...
                virtualImage = new VirtualImage(code: 'hyperv.image.morpheus.ubuntu.22.04.20260824.amd64')
                //better this later
            }
