...
//                        }
	...
	ServiceResponse<ProvisionResponse> getServerDetails(ComputeServer server) {
		...
		if (serverDetails.success == true) {
			...
			server.powerState = ComputeServer.PowerState.on
			server = saveAndGetMorpheusServer(server, true)
			//server.sshHost = server.internalIp // We are setting internalIp in appluComputeServerNetworkIp()

			def newIpAddress = serverDetails.server?.ipAddress ?: serverDetails.server?.ipAddress
	...
	ServiceResponse<ProvisionResponse> waitForHost(ComputeServer server) {
		...
		try {
			java.util.LinkedHashMap<java.lang.String, java.lang.Object> scvmmOpts = fetchScvmmConnectionDetails(server)
	...
	private java.util.LinkedHashMap<java.lang.String, java.lang.Object> fetchScvmmConnectionDetails(ComputeServer server) {
		def config = server.getConfigMap()
