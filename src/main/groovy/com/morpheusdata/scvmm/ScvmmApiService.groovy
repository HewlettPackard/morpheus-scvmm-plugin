...
class ScvmmApiService {
    ...
    private LogInterface log = PrefixedLoggerFactory.getLogger(ScvmmApiService)
    ...
    def listDatastores(Map opts) {
		...
		def fetchStorageVolumes = { offset ->
			...
			if (opts.zone.regionCode) {
				...
				// otherwise every volume known to the SCVMM server is imported. Mirrors isHostInHostGroup().
				def hostGroupPath = opts.zone.getConfigProperty('hostGroup')?.toString()?.replace("'", "''")
